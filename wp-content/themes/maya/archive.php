...
<div id="page" class="page">
    ...
    <div class="header-page marg-top-100 padd-y-75 bg-gr1-clr">
    	...
    	<div class="title-page padd-x-25 fs-clr">
			<?php 
			$tax_var = get_query_var( 'tax_query' );
			if($tax_var) {
				$tax_var = $tax_var[0]['taxonomy'];
			}
			
			if ( have_posts() ) {

				if ( is_day() ) {
					printf( __( 'Daily Archives: %s', UN ), get_the_date() );
			
				}elseif ( is_month() ) {
					printf( __( 'Monthly Archives: %s', UN ), get_the_date( _x( 'F Y', 'monthly archives date format', UN ) ) );
			
				}elseif ( is_year() ) {
					printf( __( 'Yearly Archives: %s', UN ), get_the_date( _x( 'Y', 'yearly archives date format', UN ) ) );
			
				}elseif ( $tax_var == 'post_format' ) {
					_e( 'Standard Posts', UN );
