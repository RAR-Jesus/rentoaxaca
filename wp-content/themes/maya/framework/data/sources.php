...
function vp_get_uncommons_icons()
{
	// scrape list of icons from uncommons css
	//if( false === ( $icons  = get_transient( 'vp_fontawesome_icons' ) ) )
	//{
		$pattern = '/\.(icon-(?:\w+(?:-)?)+):before\s*{\s*content/';
		$subject = file_get_contents(VP_DIR . '/public/css/uncommons.icons.css');
		
		preg_match_all($pattern, $subject, $matches, PREG_SET_ORDER);

		$icons = array();
		
		$icons[] = array('value' => 'no-icon', 'label' => __('No Icon', UN));
