...
class UN_Mega_Projects extends WP_Widget {
	...
	function UN_Mega_Projects() {
		...
		$this->WP_Widget( 'un-mega-projects', __('UN: Mega Projects', UN), $widget_ops );
		...
		if ( $qr->have_posts() ) {
			...
			while ($qr->have_posts()) { $qr->the_post();
				...
				echo '<li>';
					...
					echo get_the_post_thumbnail(get_the_ID(), 'maya-widget');
					
					echo '<span>'.get_the_date().' - '.$post_cat.'</span>';
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php echo 'Number of projects to show:'; ?></label>
		<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>

		<p>
			<label for="<?php echo $this->get_field_id( 'cat_filter' ); ?>"><?php _e('Category Filter:', UN); ?></label>
		 ...
		 <p><label for="<?php echo $this->get_field_id('orderby'); ?>"><?php echo 'Order projects by:'; ?></label>
