...
add_action( 'widgets_init', 'un_flickr_widget' );
function un_flickr_widget() {
	register_widget( 'un_flickr' );
}
class un_flickr extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'flickr-widget' );
		$control_ops = array( 'width' => 280, 'height' => 350, 'id_base' => 'flickr_widget' );
		$this->WP_Widget( 'flickr_widget','UN: Flickr Gallery', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'flickr_display' ); ?>" name="<?php echo $this->get_field_name( 'flickr_display' ); ?>" >
				<option value="latest" <?php if( $instance['flickr_display'] == 'latest' ) echo "selected=\"selected\""; else echo ""; ?>>Most recents</option>
				<option value="random" <?php if( $instance['flickr_display'] == 'random' ) echo "selected=\"selected\""; else echo ""; ?>>Maya selection</option>
			</select>
		</p>
