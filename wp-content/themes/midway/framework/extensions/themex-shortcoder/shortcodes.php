...
function themex_posts( $atts, $content = null ) {
	...
	if ($query->have_posts()) : while ( $query->have_posts() ) : $query->the_post();
		...
		$out.='<div class="post-meta"><a href="'.get_permalink().'" class="button small"><span>'.__('Read More','midway').'</span></a>';
		if(comments_open()) {
...
function themex_button( $atts, $content = null ) {	
   ...
   return '<a href="'.$url.'" target="_'.$target.'" class="button '.$size.' '.$color.'"><span>'.do_shortcode($content).'</span></a>';
}
