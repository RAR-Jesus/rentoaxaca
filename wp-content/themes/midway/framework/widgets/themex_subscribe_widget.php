...
class themex_subscribe_widget extends WP_Widget {
	...
	function __construct() {
		//Basic settings
		$settings = array( 'classname' => 'widget-subscribe', 'description' => __('Mailing list subscription form', 'midway') );

		//Controls
		$controls = array( 'width' => 300, 'height' => 300, 'id_base' => __CLASS__ );

		//Creation
		$this->WP_Widget( __CLASS__, __('Newsletter','midway'), $settings, $controls );
	...
	function widget( $args, $instance ) {
		...
		<form id="subscribe_form">			
			<input type="text" id="email" value="<?php _e('Email Address','midway'); ?>" />
		</form>
		<?php
		echo $after_widget;
	}
	...
	function updateData() {
		$email=$_POST['email'];
		if($email=='') {
			$out='<ul class="list-5 styled-list error"><li>'.__('Email field is required.','midway').'</li></ul>';
		} else if(!preg_match("/^[^@]*@[^@]*\.[^@]*$/", $email)) {
			$out='<ul class="list-5 styled-list error"><li>'.__('Email address is incorrect.','midway').'</li></ul>';
		} else {
			...
			if(!in_array($email,$mail_list)) {
				...
				$out='<ul class="list-3 styled-list success"><li>'.__('Thank you for subscribing!','midway').'</li></ul>';	
			} else {
				$out='<ul class="list-5 styled-list error"><li>'.__('This email is already in the list.','midway').'</li></ul>';
			}
		}
		echo $out;
		die();
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('text'); ?>"><?php _e('Text', 'midway'); ?>:</label>
